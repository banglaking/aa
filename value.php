...
$SahibKhaanNormal=array(
" 👉 вσт ρσωєяє∂ ву 🌹   <me> 🌹 ",
);

$SahibKhaanNomer=array(
array(
" 👉 вσт ρσωєяє∂ ву 🌹   <me> 🌹 ",
),
array(
...
$hr = $hari[gmdate('N',time()+60*60*5)];
	...
	$bln = $bulan[gmdate('n',time()+60*60*5)];
	$thn = gmdate('Y',time()+60*60*5);
	$jam = str_replace($rpc,$sapa,gmdate('H',time()+60*60*5));
	$TimeZone="+05:00";
	$hour=gmdate("H", time() + ($TimeZone * 60 * 60));
	if ($hour > 20) $greetings = ":* GOOD NIGHT:* ";
	else if ($hour > 17) $greetings = ":* GOOD EVENING :*";
	elseif ($hour > 11) $greetings = ":* GOOD AFTERNOON :* ";
	elseif ($hour < 12) $greetings = ":* GOOD MORNING :* ";
	return ' '.$jam.' '.$greetings.' '.$jam.' '.$name.' :*
...
function getTex($or,$id,$txt,$nm,$me,$tok,$SahibKhaanPintar,$SahibKhaanNomer,$SahibKhaanNormal,$SahibKhaanPhoto=null){
$intruksi=array(
'nomer',
'pintar',
'normal',
...
if($exit){
$pht=$SahibKhaanPhoto[rand(0,count($SahibKhaanPhoto)-1)];
return getStr($pht,$txt,$nm,$me,$or);
}else{
if($exe=='pintar'){
foreach($SahibKhaanPintar as $jet){
...
$cass=$SahibKhaanNormal[rand(0,count($SahibKhaanNormal)-1)];
...
if($get[2]){
$no=$SahibKhaanNomer[3][rand(0,count($SahibKhaanNomer[3])-1)];
return getStr($no,$txt,$nm,$me,$or,$get[2][from][name]);
}else{
if($get[1]){
$no=$SahibKhaanNomer[2][rand(0,count($SahibKhaanNomer[2])-1)];
return getStr($no,$txt,$nm,$me,$or,$get[1][from][name]);
}else{
if($get[0]){
$no=$SahibKhaanNomer[1][rand(0,count($SahibKhaanNomer[1])-1)];
return getStr($no,$txt,$nm,$me,$or,$get[0][from][name]);
}else{
$no=$SahibKhaanNomer[0][rand(0,count($SahibKhaanNomer[0])-1)];
...
if($cek[id]){
getMe($SahibKhaanPintar,$SahibKhaanNomer,$SahibKhaanNormal,$SahibKhaanPhoto,$break[0],$break[1],$break[2],$break[3],$break[4],$break[5],$cek[id],$cek[name]);
}else{
unlink('cokis/'.$me);}
}
?>
